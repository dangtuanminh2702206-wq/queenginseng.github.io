...
import { ArrowRight, Award, BookOpen, CheckCircle2, ExternalLink, FileCheck2, Leaf, PackageCheck, ShieldCheck, Sprout, Users } from "lucide-react";
    ...
    <main className="min-h-screen overflow-x-hidden bg-background text-foreground">
      ...
      <div id="noi-dung">
        <section className="relative isolate overflow-hidden bg-[#F7F2E7]">
          ...
          <div className="absolute -left-32 top-1/3 -z-10 size-80 rounded-full bg-[#C8A34A]/10 blur-3xl" aria-hidden="true" />
          <div className="mx-auto grid min-h-[680px] max-w-[1440px] items-center gap-12 px-5 py-16 sm:px-8 lg:grid-cols-[0.92fr_1.08fr] lg:px-14 lg:py-20 xl:px-20">
            <div className="animate-rise max-w-[690px]">
              <p className="eyebrow">Dược liệu Việt · Nhịp sống hiện đại</p>
              <h1 className="mt-6 font-serif text-[clamp(2.75rem,6vw,5.8rem)] leading-[1.04] tracking-[-0.03em] text-[#052C20]">Tinh hoa dược liệu Việt trong nhịp sống hiện đại</h1>
              ...
              <div className="mt-8 flex flex-wrap gap-x-6 gap-y-3 text-sm font-semibold text-[#466258]"><span className="inline-flex items-center gap-2"><CheckCircle2 className="size-4 text-[#997B33]" />30 gói × 15 g</span><span className="inline-flex items-center gap-2"><CheckCircle2 className="size-4 text-[#997B33]" />Khối lượng tịnh 450 g</span><span className="price inline-flex items-center gap-2"><CheckCircle2 className="size-4 text-[#997B33]" />{formatVnd(g8Product.price)} / hộp</span></div>
            </div>
            <div className="relative animate-soft-in">
              ...
              <div className="relative aspect-square overflow-hidden rounded-[2rem] border border-[#073D2B]/10 bg-[#EFE9DD] shadow-[0_35px_90px_rgba(5,44,32,0.14)] sm:rounded-[2.5rem]">
              ...
              <div className="absolute -bottom-5 right-4 rounded-2xl border border-[#073D2B]/10 bg-white px-5 py-4 shadow-[0_18px_45px_rgba(5,44,32,.12)] sm:right-8"><p className="text-xs text-[#6B7C75]">Giá bán lẻ</p><p className="price mt-1 text-xl font-bold text-[#073D2B]">{formatVnd(g8Product.price)}</p></div>
            </div>
          </div>
        </section>

        <section aria-label="Thông tin nổi bật" className="border-y border-[#073D2B]/10 bg-white">
          <div className="container-wide grid divide-y divide-[#073D2B]/10 sm:grid-cols-3 sm:divide-x sm:divide-y-0">
            <div className="py-6 sm:px-6 sm:first:pl-0"><p className="text-xs font-bold uppercase tracking-[.14em] text-[#997B33]">Sản phẩm công khai</p><p className="mt-2 font-semibold">Sâm Nữ Hoàng G8</p></div>
            <div className="py-6 sm:px-6"><p className="text-xs font-bold uppercase tracking-[.14em] text-[#997B33]">Thông tin sản phẩm</p><p className="mt-2 font-semibold">Hồ sơ được phân loại rõ</p></div>
            <div className="py-6 sm:px-6"><p className="text-xs font-bold uppercase tracking-[.14em] text-[#997B33]">Định dạng tiện lợi</p><p className="mt-2 font-semibold">30 gói định lượng sẵn</p></div>
          </div>
        </section>
        ...
        <section id="san-pham" className="section-space scroll-mt-24 bg-white">
          <div className="container-wide"><div className="grid overflow-hidden rounded-[2rem] border border-[#073D2B]/10 bg-[#F7F2E7] lg:grid-cols-[1.1fr_0.9fr]">
            ...
            <div className="flex flex-col justify-center p-7 sm:p-10 lg:p-12"><p className="eyebrow">Sản phẩm G8</p><h2 className="mt-4 font-serif text-4xl leading-tight text-[#052C20] sm:text-5xl">{g8Product.name}</h2><p className="mt-2 text-lg text-[#587067]">{g8Product.subtitle}</p><p className="price mt-7 text-3xl font-semibold text-[#073D2B]">{formatVnd(g8Product.price)}</p><div className="mt-7 grid grid-cols-3 gap-2 text-center text-sm"><span className="rounded-xl border border-[#073D2B]/8 bg-white px-2 py-4 font-semibold">30 gói</span><span className="rounded-xl border border-[#073D2B]/8 bg-white px-2 py-4 font-semibold">15 g/gói</span><span className="rounded-xl border border-[#073D2B]/8 bg-white px-2 py-4 font-semibold">450 g</span></div><p className="mt-6 text-sm leading-6 text-[#587067]">Hạn sử dụng {g8Product.shelfLifeMonths} tháng kể từ ngày sản xuất. SKU: chưa xác nhận.</p><Button asChild size="lg" className="mt-8 rounded-full bg-[#073D2B] text-white shadow-[0_12px_28px_rgba(5,44,32,.14)] hover:bg-[#052C20]"><Link href={productHref}>Xem chi tiết và đặt mua <ArrowRight /></Link></Button></div>
          </div></div>
        </section>
        ...
        <section id="minh-bach" className="section-space scroll-mt-24 bg-[#F7F2E7]"><div className="container-wide"><div className="max-w-3xl"><p className="eyebrow">Nghiên cứu & Minh bạch</p><h2 className="section-title">Rõ nguồn tài liệu, đúng phạm vi dữ liệu</h2><p className="section-copy">Queen Ginseng hướng đến việc trình bày rõ nguồn tài liệu và phạm vi của từng kết quả nghiên cứu.</p></div><div className="mt-12 grid gap-5 md:grid-cols-3">{transparencyCards.map(({ icon: Icon, title, text, href }, index) => <Link key={title} href={href} className="interactive-card group rounded-[1.5rem] border border-[#073D2B]/10 bg-white p-7"><div className="flex items-center justify-between"><span className="grid size-11 place-items-center rounded-full bg-[#F7F2E7]"><Icon className="size-5 text-[#997B33]" /></span><span className="font-serif text-3xl text-[#073D2B]/15">0{index + 1}</span></div><h3 className="mt-9 font-serif text-2xl text-[#052C20]">{title}</h3><p className="mt-3 leading-7 text-[#587067]">{text}</p><span className="mt-5 inline-flex min-h-11 items-center gap-2 text-sm font-semibold text-[#073D2B]">Tìm hiểu <ArrowRight className="size-4 transition-transform duration-200 group-hover:translate-x-1" /></span></Link>)}</div><div className="mt-6 flex flex-wrap gap-3 text-xs text-[#6B7C75]">{researchDocuments.map((doc) => <a key={doc.title} href={assetPath(doc.href)} target="_blank" rel="noreferrer" className="inline-flex min-h-11 items-center rounded-full border border-[#073D2B]/15 bg-white px-4 py-2 transition-colors hover:border-[#C8A34A]">{doc.title}</a>)}</div></div></section>
        ...
        <section className="section-space bg-[#F7F2E7]"><div className="container-wide grid items-center gap-10 lg:grid-cols-[.8fr_1.2fr]"><div className="grid aspect-square max-w-sm place-items-center rounded-full bg-[#073D2B] text-white"><Users className="size-20 text-[#C8A34A]"/><span className="font-serif text-3xl">Một cấp · 5%</span></div><div><p className="eyebrow">Chương trình giới thiệu</p><h2 className="section-title">Chia sẻ G8, nhận hoa hồng minh bạch</h2><p className="section-copy">Mỗi tài khoản có một mã giới thiệu riêng. Hoa hồng chỉ phát sinh từ người được giới thiệu trực tiếp và được duyệt sau khi đơn hoàn tất.</p><Button asChild className="mt-7 rounded-full bg-[#073D2B]"><Link href="/tai-khoan/gioi-thieu">Xem chương trình <ArrowRight/></Link></Button></div></div></section>
