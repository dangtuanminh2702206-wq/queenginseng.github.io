    ...
    <div className="mt-5">
      ...
      <div className="mt-3 flex flex-wrap gap-3">
        <div className="flex h-14 items-center rounded-full border border-[#073D2B]/20 bg-white p-1">
          <button type="button" onClick={() => setQuantity(Math.max(1, quantity - 1))} className="grid size-11 place-items-center rounded-full hover:bg-[#F7F2E7]" aria-label="Giảm số lượng"><Minus className="size-4" /></button>
          ...
          <button type="button" onClick={() => setQuantity(Math.min(99, quantity + 1))} className="grid size-11 place-items-center rounded-full hover:bg-[#F7F2E7]" aria-label="Tăng số lượng"><Plus className="size-4" /></button>
        </div>
      ...
      <p className="sr-only" role="status">{added ? `${quantity} hộp đã thêm vào giỏ` : ""}</p>
